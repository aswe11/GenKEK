...
TPCDetectorConstruction::Construct( void )
{
  ...
  auto world_solid = new G4Box( "WorldSolid", 1.*m/2, 1.2*m/2, 1.*m/2 );
  ...
  G4String filename = "hyptpcGeo_target";
  filename += target_material;
  filename += ".gdml";
  G4cout << filename <<" is generated"<< G4endl;
  parser.Write(filename.data(), world_pv);
...
TPCDetectorConstruction::ConstructHypTPC( void )
{
  ...
  // HypTPC
  {
    ...
    m_tpc_lv->SetVisAttributes( G4Colour::Gray() );
  ...
  // Field Cage
  {
    ...
    auto rot = new G4RotationMatrix;
    rot->rotateX( 90.*deg );
    new G4PVPlacement( rot, tpc_pos, fc_lv, "FieldCagePV", m_world_lv, false, 0 );
